...
void ZmpCalculationFunc::ZmpCalculationResult()
{
	...
	zmp_fz_point_x = (  cf_px_l*cf_fz_l + cf_px_r*cf_fz_r + cf_ty_l + cf_ty_r )/(cf_fz_l + cf_fz_r);
	zmp_fz_point_y = (  cf_py_l*cf_fz_l + cf_py_r*cf_fz_r + cf_tx_l + cf_tx_r )/(cf_fz_l + cf_fz_r);

	zmp_fy_point_x = (  cf_px_l*cf_fy_l + cf_px_r*cf_fy_r + cf_tz_l + cf_tz_r )/(cf_fy_l + cf_fy_r);
	zmp_fy_point_z = (  cf_pz_l*cf_fy_l + cf_pz_r*cf_fy_r + cf_tx_l + cf_tx_r )/(cf_fy_l + cf_fy_r);

	zmp_fx_point_y = (  cf_py_l*cf_fx_l + cf_py_r*cf_fx_r + cf_tz_l + cf_tz_r )/(cf_fx_l + cf_fx_r);
	zmp_fx_point_z = (  cf_pz_l*cf_fx_l + cf_pz_r*cf_fx_r + cf_ty_l + cf_ty_r )/(cf_fx_l + cf_fx_r);
}
ZmpCompensationFunc::ZmpCompensationFunc()
{
	parse_margin_data();
...
void ZmpCompensationFunc::parse_margin_data()
{
...
void ZmpCompensationFunc::centerOfPressureReferencePoint(std::string turn_type, double cur_l_point_x, double cur_l_point_y, double cur_l_point_z, double cur_r_point_x, double cur_r_point_y, double cur_r_point_z, double current_control_value)
{
	reference_point_Fz_x = 0;
	reference_point_Fz_y = 0;
}
void ZmpCompensationFunc::centerOfPressureCompensationFz(double current_point_x, double current_point_y)
{
...
void ZmpCompensationFunc::centerOfPressureCompensationFy(double current_point_x, double current_point_z)
{
...
void ZmpCompensationFunc::centerOfPressureCompensationFx(double current_point_y, double current_point_z)
{
